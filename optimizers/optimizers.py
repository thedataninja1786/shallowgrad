...
class SGD:
  ...
  def step(self):
    for l in self.layers:
      ...
      if l.use_bias: l.bias -= self.lr * l.grad_bias
...
class RMSprop:
  ...
  def step(self):
    for i,l in enumerate(self.layers):
      ...
      self.v_bias[i] = self.beta * self.v_bias[i] + (1 - self.beta) * np.square(l.grad_bias)
      l.weights -= self.lr * l.grad / (np.sqrt(self.v[i]) + self.e)
      if l.use_bias: l.bias -= self.lr * l.grad_bias / (np.sqrt(self.v_bias[i]) + self.e)
